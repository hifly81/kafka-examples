...
Images are downloaded from _confluentinc_ and are based on _Confluent 7.3.0 Community licensed (kafka version 3.3.0):

* Zookeeper: confluentinc/cp-zookeeper:7.3.0
* Kafka: confluentinc/cp-kafka:7.3.0
* Schema Registry: confluentinc/cp-schema-registry:7.3.0
* Connect: custom image based on confluentinc/cp-kafka-connect-base:7.3.0
...
[source,bash]
----
docker-compose -f kafka-oauth-kip-768/docker-compose-idp.yml down --volumes
docker-compose -f kafka-oauth-kip-768/docker-compose-oauth.yml down --volumes
----
...
[source,bash]
----
docker-compose -f kafka-distributed-tracing/docker-compose-tracing.yml up -d
----
...
[source,bash]
----
docker-compose -f kafka-distributed-tracing/docker-compose-tracing.yml down --volumes
----
...
[source,bash]
----
$KAFKA_HOME/bin/kafka-console-producer --bootstrap-server localhost:9092 --topic test --producer.config ./src/main/resources/producer.properties
----
...
[source,bash]
----
$KAFKA_HOME/bin/kafka-console-consumer --bootstrap-server localhost:9092 --topic test --from-beginning --consumer.config ./src/main/resources/consumer.properties
----
...
This example shows how to create a custom producer interceptor. This class will mask a sensitive info on producer record (credit card).
