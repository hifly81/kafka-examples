...
Images are downloaded from _confluentinc_ and are based on _Confluent 7.0.1_ version:
...
This example shows how to configure kafka to use SASL/OAUTHBEARER autehtication with Support for OIDC.
...
Add these properties to your kafka server properties and start kafka:
...
Run a kafka producer test using the _client-oauth-properties_ (add your client_secret into the file) and listener on port 9093:

[source,bash]
----
/home/hifly/Software/kafka_2.12-3.1.0/bin/kafka-producer-perf-test.sh --topic my_topic --num-records 50 --throughput 10 --record-size 1 --producer-props bootstrap.servers=localhost:9093  --producer.config kafka-oauth-kip-768/client-oauth.properties
----
