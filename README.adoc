...
Images are downloaded from _confluentinc_ and are based on _Confluent 7.2.0_ Community licensed (kafka version 3.2.0):
...
java -javaagent:kafka-distributed-tracing/app/opentelemetry-javaagent.jar -Dotel.instrumentation.kafka.enabled=true -Dotel.javaagent.debug=true -jar kafka-distributed-tracing/app/kafka-streams-0.0.1-SNAPSHOT.jar
----
