...
To run a cluster using Docker, run the _docker-compose-kraft.yml_ file available in the root directory.

Images are downloaded from _confluentinc_ and are based on _Confluent 7.4.0 Community_ licensed (Apache Kafka® version 3.4.0):
...
To run a cluster using Docker in Zookeeper mode, run the _docker-compose.yml_ file available in the root directory.
...
To launch the examples run cluster on port 9092:
...
Run a producer sending data to listener port 9092:
...
Run 3 different consumer instances (from 3 different shells) belonging to the same consumer group:
...
Consumers will start reading messages from partitions, for example:
...
It uses _com.hortonworks.registries.schemaregistry.serdes.avro.kafka.KafkaAvroSerializer_ for value and a GenericRecord.
...
Implementation of a producer and a consumer using Avro Specific Record for serializing and deserializing.
...
Verify compatibility for subject cars-value:
...
Run a consumer perf test and see how throttling applies:
