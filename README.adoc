...
=== Setup a Kafka cluster with Confluent for Kubernetes Operator
...
Option 1: Install Confluent components: 1 zk, 3 brokers, 1 connect, 1 ksqldb, 1 schema registry:
...
Option 2: Install Confluent components: 1 zk, 3 brokers:
...
Run a consumer that will read messages from replica broker in rack dc2:
