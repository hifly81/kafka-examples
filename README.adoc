...
Enable shared groups in kafka cluster:
...
[source,bash]
----
docker exec broker /opt/kafka/bin/kafka-topics.sh --bootstrap-server broker:9092 --create --topic queue-topic --replication-factor 1
----
...
Consumers will all be in the same share group and the produced records will be distributed among them depite the number of partitions set for the topic, in this example 1.
