...
To run a single-node cluster using Docker, run the link:docker-compose.yml[docker-compose.yml] file available in the root directory. In addition, it also presents a container with link:https://github.com/edenhill/kcat[kcat] on board:
...
Create topic _topic-regional_ and assign partition leaderships only on broker 1 and 3 (dc1 and dc3):
...
Run a consumer on _topic1_ topic and consumer records:
