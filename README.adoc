...
The script will create all the required security files (certs, trusstores, keystores) for broker and client application in _mtls-listener/ssl_ folder.
...
IMPORTANT: mTLS listner has been configured with hostname verification: _ssl.endpoint.identification.algorithm= https_

client CN is: _CN=client,OU=Hifly,O=Hifly,L=Rome,ST=Lazio,C=IT_

[source,bash]
----
kafka-console-producer --broker-list broker:9092 --topic topic1 --producer.config mtls-listener/client.properties --property "parse.key=true" --property "key.separator=:"
1:test
2:test2
----
