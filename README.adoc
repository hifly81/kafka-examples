...
Apache Kafka® docker images are downloaded from Docker Hub link:https://hub.docker.com/r/apache/kafka[_apache/kafka_].
...
[source,bash]
----
kubectl exec --stdin --tty kafka-0 -- kafka-topics --bootstrap-server localhost:9092 --create --topic test-1
----
...
[source,bash]
----
kubectl exec --stdin --tty kafka-0 -- kafka-topics --bootstrap-server localhost:9092 --list
----
...
[source,bash]
----
kubectl exec --stdin --tty kafka-0 -- kafka-topics --bootstrap-server localhost:9092 --topic test-1 --describe
----
...
[source,bash]
----
kubectl exec --stdin --tty kafka-0 -- kafka-producer-perf-test --num-records 1000000 --record-size 1000 --throughput -1 --topic test-1 --producer-props bootstrap.servers=localhost:9092
----
...
[source,bash]
----
kubectl exec --stdin --tty kafka-0 -- kafka-console-consumer --bootstrap-server localhost:9092 --topic test-1 --from-beginning
----
...
=== Manage Confluent for Kubernetes using ArgoCD

Confluent For Kubernetes deployment with ArgoCD. Easily install and sync Confluent Platform components (kraft, kafka, schema registry, connect, ksql, control center) and resources (topics, acls, rbac, schemas, connectors).
...
Resources are stored in GitHub repo:
https://github.com/hifly81/cfk-deployment

==== Deploy Confluent Platform using ArgoCD
...
Create secret with confluent license _(you need to obtain a Confluent license first)_:
...
Queues for Kafka introduce a queue‑like consumption model built on share consumers and share groups. Instead of assigning each partition to a single consumer (as with traditional consumer groups), multiple share consumers in the same share group can process records from the same partition concurrently, allowing parallelism beyond the partition count. Each record is individually acknowledged, controlled by _share.acknowledgement.mode_ (implicit by default, or explicit when you want fine‑grained ACCEPT/RELEASE/REJECT semantics).

In order to test the feature, enable shared groups in kafka cluster:

[source,bash]
----
docker exec broker /opt/kafka/bin/kafka-features.sh --bootstrap-server broker:9092 upgrade --feature share.version=1
----

Create topic _queue-topic_:

[source,bash]
----
docker exec broker /opt/kafka/bin/kafka-topics.sh --bootstrap-server broker:9092 --create --partitions 1 --topic queue-topic --replication-factor 1
----
...
----
cd kafka-consumer
mvn clean compile && mvn exec:java -Dexec.mainClass="org.hifly.kafka.demo.consumer.queue.Runner"
----
...
Run a producer perf test to send messages to _queue-topic_:

[source,bash]
----
docker exec -it broker /opt/kafka/bin/kafka-producer-perf-test.sh --topic queue-topic --num-records 10000 --throughput -1 --record-size 2000 --producer-props bootstrap.servers=broker:9092
----

Consumers will all be in the same share group and the produced records will be distributed among them despite the number of partitions set for the topic, in this example 1.

It is possible to run a queue consumer using the CLI as well:

[source,bash]
----
docker exec broker /opt/kafka/bin/kafka-console-share-consumer.sh --bootstrap-server broker:9092 --topic queue-topic --group test-share-group --property print.offset=true --property print.partition=true --property print.timestamp=true
----


Useful commands to inspect a shared group:

[source,bash]
----
docker exec broker /opt/kafka/bin/kafka-share-groups.sh --bootstrap-server broker:9092 --list
----

[source,bash]
----
docker exec broker /opt/kafka/bin/kafka-share-groups.sh --bootstrap-server broker:9092 --describe --group group-1
----

[source,bash]
----
docker exec broker /opt/kafka/bin/kafka-share-groups.sh --bootstrap-server broker:9092 --describe --group group-1 --members
----

[source,bash]
----
docker exec broker /opt/kafka/bin/kafka-share-groups.sh --bootstrap-server broker:9092 --describe --group group-1 --state
----
...
Implementation of producers and consumers using Avro link:https://avro.apache.org/docs/1.11.1/api/java/org/apache/avro/specific/SpecificRecord.html[_SpecificRecord_] for serializing and deserializing.
...
mvn clean compile package && mvn exec:java -Dexec.mainClass="org.hifly.kafka.demo.avro.CarConsumer"
----
...
----
cd confluent-avro-specific-record
mvn clean compile package && mvn exec:java -Dexec.mainClass="org.hifly.kafka.demo.avro.CarProducer"
----

In this additional example, producer will create messages using DECIMAL types with precision and scale.
An AVRO schema will be set for both, key and value.

Create _CDC_topic_ topic:

[source,bash]
----
docker exec -it broker kafka-topics --bootstrap-server broker:9092 --create --topic CDC_topic --replication-factor 1 --partitions 1
----

Avro schema for value link:confluent-avro-specific-record/src/main/resources/cdc-value.avsc[_cdc-value.avsc_]:
...
----
curl -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" \
--data @confluent-avro-specific-record/src/main/resources/cdc-value.avsc \
http://localhost:8081/subjects/CDC_topic-value/versions
----

Avro schema for key link:confluent-avro-specific-record/src/main/resources/cdc-key.avsc[_cdc-key.avsc_]:
...
----
curl -X POST -H "Content-Type: application/vnd.schemaregistry.v1+json" \
--data @confluent-avro-specific-record/src/main/resources/cdc-key.avsc \
http://localhost:8081/subjects/CDC_topic-key/versions
----
...
----
cd confluent-avro-specific-record
mvn clean compile package && mvn exec:java -Dexec.mainClass="org.hifly.kafka.demo.avro.CDCProducer"
----
...
=== CDC with Debezium MongoDB Source Connector and SMTs

Folder: link:cdc-debezium-mongo-smt/[cdc-debezium-mongo-smt]

Usage of link:https://debezium.io/documentation/reference/stable/connectors/mongodb.html[Debezium Source Connector for MongoDB] to send updates into a topic.

Usage of link:https://www.mongodb.com/docs/kafka-connector/current/sink-connector[MongoDB Sink Connector] to send updates into a new mongo collection.

This example will use out of the SMTs to transform the original record.

Run environment:

[source,bash]
----
scripts/bootstrap-cdc-mongo-smt.sh
----
...
Script will also insert a document into _outbox_ database and _Order_ collection:
...
Script will deploy a source connector.
If the record value contains a field named "operation", that field’s value will be copied into a record header named "operation".
If the value does not contain that field, no header will be added.

[source,json]
----
{
  "name": "mongo-debezium-transforms-connector",
  "config": {
    "connector.class": "io.debezium.connector.mongodb.MongoDbConnector",
    "mongodb.connection.string": "mongodb://mongo:27017/?replicaSet=rs0",
    "topic.prefix": "mvp-order",
    "database.include.list" : "outbox",
    "collection.include.list" : "outbox.Order",
    "mongodb.user" : "data-platform-cdc",
    "mongodb.password" : "password",
    "value.converter": "org.apache.kafka.connect.json.JsonConverter",
    "value.converter.schemas.enable": "false",
    "transforms": "unwrap,addOpHeader",
    "transforms.unwrap.type": "io.debezium.connector.mongodb.transforms.ExtractNewDocumentState",
    "transforms.unwrap.drop.tombstones": "false",
    "transforms.unwrap.operation.header": "false",
    "transforms.unwrap.delete.handling.mode": "drop",
    "transforms.unwrap.array.encoding": "array",
    "transforms.addOpHeader.type": "org.apache.kafka.connect.transforms.HeaderFrom$Value",
    "transforms.addOpHeader.headers": "operation",
    "transforms.addOpHeader.fields": "operation",
    "transforms.addOpHeader.operation": "copy"
  }
}
----

A sink connector will read messages from _mvp-order.outbox.Order_ topic and apply additional transformations before inserting the new document into a new collection on MongoDB, named _account_.
...
----
{
  ...
  "config": {
    ...
    "database": "outbox",
    "collection": "account",
    "transforms": "ValueToKey,removeFields",
    "transforms.ValueToKey.type": "org.apache.kafka.connect.transforms.ValueToKey",
    "transforms.ValueToKey.fields": "orderId",
    ...
    "transforms.removeFields.blacklist": "operation"
  }
}
----

Check the documents on _account_ collection using:
...
[source,bash]
----
scripts/tear-down-cdc-mongo-smt.sh
----
...
[source,bash]
----
kubectl exec --stdin --tty kafka-0 -- kafka-topics --bootstrap-server localhost:9092 --create --topic words-input-topic --partitions 6
kubectl exec --stdin --tty kafka-0 -- kafka-topics --bootstrap-server localhost:9092 --create --topic words-counter-output-topic --partitions 6
----
...
[source,bash]
----
kubectl exec --stdin --tty kafka-0 -- kafka-console-producer --bootstrap-server localhost:9092 --topic words-input-topic --property "key.separator=:" --property "parse.key=true"

# Then type the following data:
# user1:hello world
...
[source,bash]
----
kubectl exec --stdin --tty kafka-0 -- kafka-console-consumer --bootstrap-server localhost:9092 --topic words-counter-output-topic --from-beginning --property "print.key=true" --property "key.separator=:" --value-deserializer "org.apache.kafka.common.serialization.LongDeserializer"
----
...
Additional scripts are available:
 ...
 - _kafka-acl-dump.sh_ --> List Kafka ACLs via Admin API, outputs either raw tool output or CSV.
 - _kafka-scram-users-dump.sh_ --> List Kafka users that have SCRAM credentials (SCRAM-SHA-256 / SCRAM-SHA-512), outputs CSV
...
[source,bash]
----
docker exec -it broker /opt/kafka/bin/kafka-console-producer.sh --bootstrap-server broker:9093 --producer.config /tmp/client.properties --topic topic1 --property "parse.key=true" --property "key.separator=:"
1:test
2:test2
----
...
[source,bash]
----
docker exec broker kafka-producer-perf-test --topic my_topic --num-records 50 --throughput 10 --record-size 1 --producer-props bootstrap.servers=localhost:9093 --producer.config kafka-oauth-kip-768/client-oauth.properties
----
...
[source,bash]
----
docker exec broker kafka-run-class kafka.tools.JmxTool --jmx-url service:jmx:rmi:///jndi/rmi://localhost:9101/jmxrmi --object-name kafka.controller:type=KafkaController,name=OfflinePartitionsCount --one-time true
----
...
[source,bash]
----
docker exec broker kafka-run-class kafka.tools.JmxTool --jmx-url service:jmx:rmi:///jndi/rmi://localhost:9101/jmxrmi --object-name kafka.server:name=UnderMinIsrPartitionCount,type=ReplicaManager --one-time true
----
...
[source,bash]
----
docker exec broker kafka-run-class kafka.tools.JmxTool --jmx-url service:jmx:rmi:///jndi/rmi://localhost:9101/jmxrmi --object-name kafka.controller:type=KafkaController,name=ActiveControllerCount
----
...
[source,bash]
----
docker exec broker kafka-producer-perf-test --topic topic-perf --num-records 1000000 --record-size 2000 --throughput -1 --producer-props bootstrap.servers=host.docker.internal:1999 --print-metrics
----
