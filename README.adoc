...
Usage of link:https://debezium.io/documentation/reference/stable/connectors/mongodb.html[Debezium Source Connector for MongoDB] to send updates into a topic. This example will use out of the SMTs to transform the original record.
...
Run a consumer on _mvp-order.outbox.Order_ topic and see the records _(headers - key - value)_:

[source,bash]
----
docker exec -it broker kafka-console-consumer --bootstrap-server broker:9092 --topic mvp-order.outbox.Order --from-beginning --property print.key=true --property print.headers=true
----
