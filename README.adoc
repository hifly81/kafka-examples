...
Usage of link:https://debezium.io/documentation/reference/stable/connectors/mongodb.html[Debezium Source Connector for MongoDB] to send updates into a topic. This example will use Debezium Event Router to implement a use case suitable for Outbox.
...
Run a consumer on _CONSUMER_LOAN_ topic and see the records:

[source,bash]
----
kafka-console-consumer --bootstrap-server localhost:9092 --topic CONSUMER_LOAN --from-beginning --property print.key=true --property print.headers=true
----

[source,bash]
----
id:012313,aggregateType:Consumer Loan,eventTime:2024-08-20T09:42:02.665Z,type:INSTALLMENT_PAYMENT,id:1	012313	{"schema":{"type":"struct","fields":[{"type":"string","optional":true,"field":"amount"}],"optional":true,"name":"io.debezium.data.Json.payload"},"payload":{"amount":"200000"}}
----
